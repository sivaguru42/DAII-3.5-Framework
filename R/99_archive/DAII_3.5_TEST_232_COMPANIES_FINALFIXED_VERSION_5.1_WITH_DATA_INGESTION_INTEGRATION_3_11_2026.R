...
patent_data <- pull_patent_data(company_map)
cat(sprintf("   ✅ Pulled patent data for %d companies\n", nrow(patent_data)))

# -----------------------------------------------------------------------------
# 0.7 Load Bloomberg cached data (development)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
if (exists("pull_msci_data")) {
  cat("\n📊 Pulling MSCI index data...\n")
  msci_data <- pull_msci_data(company_map)
  cat(sprintf("   ✅ Pulled MSCI data for %d companies\n", nrow(msci_data)))
} else {
  msci_data <- NULL
...
# Join MSCI data if available
if (!is.null(msci_data)) {
  master_snapshot <- master_snapshot %>%
    left_join(msci_data, by = "Ticker")
}
