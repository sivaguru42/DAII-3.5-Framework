...
# -----------------------------------------------------------------------------
# NEW: Single utils file for all database connections
source("C:\\Users\\sganesan\\OneDrive - dumac.duke.edu\\DAII\\R\\scripts\\utils\\R_utils_warehouse_connection.r")  # ← ADD THIS

# Keep all these - they now use the new utils internally
source("C:\\Users\\sganesan\\OneDrive - dumac.duke.edu\\DAII\\R\\scripts\\ingest\\R_ingest_build_company_resolution.r")
source("C:\\Users\\sganesan\\OneDrive - dumac.duke.edu\\DAII\\R\\scripts\\ingest\\R_ingest_pull_rd_history.r")
source("C:\\Users\\sganesan\\OneDrive - dumac.duke.edu\\DAII\\R\\scripts\\ingest\\R_ingest_pull_current_holdings.r")
source("C:\\Users\\sganesan\\OneDrive - dumac.duke.edu\\DAII\\R\\scripts\\ingest\\R_ingest_pull_daily_ratios.r")
source("C:\\Users\\sganesan\\OneDrive - dumac.duke.edu\\DAII\\R\\scripts\\ingest\\R_ingest_ingest_patentsview.r")

# Optional: Bloomberg dev pull (if needed)
# source("C:\\Users\\sganesan\\OneDrive - dumac.duke.edu\\DAII\\R\\scripts\\ingest\\R_ingest_bloomberg_dev_pull.r")


# -----------------------------------------------------------------------------
# 0.x Load Bloomberg cached data (development)
# -----------------------------------------------------------------------------
cat("📈 Loading cached Bloomberg monthly prices...\n")
bloomberg_cache_file <- "C:\\Users\\sganesan\\OneDrive - dumac.duke.edu\\DAII\\data\\raw\\bloomberg_monthly_prices.rds"

if(file.exists(bloomberg_cache_file)) {
  bloomberg_data <- readRDS(bloomberg_cache_file)
  cat(sprintf("   ✅ Loaded cached Bloomberg data: %d rows\n", nrow(bloomberg_data)))
  
  # Check the structure of bloomberg_data
  cat("   📊 Data structure:\n")
  cat("   Columns:", paste(names(bloomberg_data)[1:min(5, ncol(bloomberg_data))], collapse=", "), "...\n")
  
  # Process into format needed for pipeline - handle different possible column names
  bloomberg_prices <- bloomberg_data %>%
    # Handle different possible column structures
    {
      # If data is in list format from bdh() with multiple securities
    ...
    rename_with(~ ifelse(. == "date", "date", 
                         ifelse(. == "PX_LAST", "price", 
                                ifelse(. == "ticker", "ticker", .)))) %>%
    group_by(ticker) %>%
    arrange(date) %>%
    mutate(
      monthly_return = price / lag(price) - 1,
      volatility_12m = sd(monthly_return, na.rm = TRUE) * sqrt(12),  # Annualized from monthly
      date = as.Date(date)
    ) %>%
    ungroup() %>%
    filter(!is.na(monthly_return))  # Remove first row with NA return
  
  cat(sprintf("   ✅ Processed %d price records\n", nrow(bloomberg_prices)))
  cat(sprintf("   📅 Date range: %s to %s\n", 
              min(bloomberg_prices$date), 
              max(bloomberg_prices$date)))
  
} else {
  cat("   ⚠️ No cached Bloomberg data found. Run R_ingest_bloomberg_dev_pull.r first\n")
  cat("   To create this file:\n")
  cat("   1. Open R_ingest_bloomberg_dev_pull.r\n")
  cat("   2. Run it once to pull monthly data\n")
  cat("   3. Re-run this pipeline\n")
  bloomberg_data <- NULL
  bloomberg_prices <- NULL
}

# -----------------------------------------------------------------------------
# 0.2 Build company resolution table (Ticker → RepNo)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
# 0.6 Pull patent data from PatentsView API
# -----------------------------------------------------------------------------
cat("\n🔬 Pulling AI patent data from PatentsView...\n")
# Load company name mapping (you'll need to create this)
company_names <- data.frame(
  ticker = company_map$Ticker,
  search_name = gsub(" US$", "", company_map$Ticker)  # Basic mapping - improve this
)

patent_data <- batch_get_patents(company_names$search_name[1:10])  # Start with 10
cat(sprintf("   ✅ Pulled patent data for %d companies\n", nrow(patent_data)))

# -----------------------------------------------------------------------------
# 0.7 Create master dataset (one row per company)
# -----------------------------------------------------------------------------
cat("\n🔄 Creating master company snapshot...\n")

# Start with R&D history (most recent year)
latest_rd <- rd_history %>%
  group_by(Ticker) %>%
  slice_max(fiscal_year_end, n = 1) %>%
  ungroup() %>%
  select(Ticker, rd_expense, revenue, fiscal_year_end)

# Join with current holdings
master_snapshot <- latest_rd %>%
  full_join(current_holdings, by = "Ticker") %>%
  full_join(daily_ratios %>% 
              filter(as_of_date == max(as_of_date)) %>%
              select(Ticker, MKTCAP_USD, PE_ratio, Beta_3Y_weekly),
            by = "Ticker") %>%
  left_join(patent_data, by = c("Ticker" = "ticker")) %>%
  mutate(
    # Set defaults for missing data
    rd_expense = ifelse(is.na(rd_expense), MKTCAP_USD * 0.03, rd_expense),
    patent_activity = ifelse(is.na(patent_count), 0, patent_count),
    in_portfolio = !is.na(fund_weight),
    rd_intensity = rd_expense / MKTCAP_USD,
    fund_weight = ifelse(is.na(fund_weight), 1/nrow(.), fund_weight)
  )
...
cat(sprintf("   📊 With R&D data: %d\n", sum(!is.na(master_snapshot$rd_expense))))
cat(sprintf("   📊 With patent data: %d\n", sum(master_snapshot$patent_activity > 0)))

# Save snapshot for pipeline
snapshot_file <- file.path(raw_dir, "live_company_snapshot.csv")
write.csv(master_snapshot, snapshot_file, row.names = FALSE)
cat("\n✅ Live snapshot saved to:", snapshot_file, "\n")

# -----------------------------------------------------------------------------
# 0.8 Set daii_raw_data for Module 0.5
# -----------------------------------------------------------------------------
...
# =============================================================================
cat(paste(rep("=", 80), collapse = ""), "\n")
cat("🏦 SECTION 2: LOADING HOLDINGS DATA\n")
cat(paste(rep("=", 80), collapse = ""), "\n\n")
...
load_panel_data <- function() {
  ...
  cat("   Panel should contain ~232 tickers × 1500 days\n")
...
if(is.null(panel_data)) {
  cat("\n⚠️  No panel data available. Creating synthetic panel from fundamentals.\n")
...
if(file.exists(holdings_file)) {
  ...
  if(sum(!daii_scored$in_portfolio) > 0) {
    cat("\n📋 Sample of non-portfolio companies:\n")
...
cat("\n✅ Holdings merge complete\n")

# After Section 3, add:
holdings_only <- setdiff(holdings_summary$Ticker, fundamentals$Ticker)
cat("Tickers in holdings but not in fundamentals:\n")
print(holdings_only)
# You'll likely see ETFs and bonds
...
# ===== ADD MERGE STEP HERE =====
# Merge AI cube data back to main dataset
daii_scored <- daii_scored %>%
...
cat("\n   Verifying merge:\n")
cat("   strategic_profile in daii_scored:", "strategic_profile" %in% names(daii_scored), "\n")
if("strategic_profile" %in% names(daii_scored)) {
  cat("   Sample values:", paste(head(daii_scored$strategic_profile, 3), collapse=", "), "\n")
} else {
  cat("   ⚠️ Merge failed - strategic_profile not found!\n")
}

# ===== THEN ADD PROFILE FILLING HERE =====
# Fill any missing strategic profiles
cat("\n   Checking for missing strategic profiles...\n")
na_profiles <- sum(is.na(daii_scored$strategic_profile))
if(na_profiles > 0) {
  cat("   ⚠️ Found", na_profiles, "missing strategic profiles. Adding defaults.\n")
  daii_scored <- daii_scored %>%
    mutate(
      strategic_profile = ifelse(is.na(strategic_profile),
                                 case_when(
                                   ai_quartile == 4 & innovation_quartile == 4 ~ "AI Pioneer",
  ...
  # Verify fix worked
  na_left <- sum(is.na(daii_scored$strategic_profile))
  cat("   ✅ After fix:", na_left, "NAs remaining\n")
} else {
  cat("   ✅ No missing profiles found\n")
}
...
daii_scored <- daii_scored %>%
  left_join(ai_cube[, c("ticker", "anomaly_score", "is_anomaly", "strategic_profile")], 
            by = "ticker")
# Ensure anomaly scores are propagated to all rows
cat("   Checking anomaly score propagation...\n")
na_count <- sum(is.na(daii_scored$anomaly_score))
if(na_count > 0) {
  cat("   ⚠️ Found", na_count, "NA values in anomaly_score. Filling with defaults.\n")
  daii_scored <- daii_scored %>%
    mutate(
      anomaly_score = ifelse(is.na(anomaly_score), 
                             median(anomaly_score, na.rm = TRUE), 
                             anomaly_score),
...
# ===== INSERT FINAL DEDUPLICATION HERE =====
cat("\n🔄 FINAL DEDUPLICATION: Ensuring one row per company...\n")
cat("   Before final dedupe:", nrow(daii_scored), "rows\n")
...
cat("   After final dedupe:", nrow(daii_scored), "unique companies\n")

# Clean up duplicate columns from multiple merges
cat("\n🧹 Cleaning up duplicate columns...\n")

# Keep the most recent strategic_profile (from the last merge)
if("strategic_profile.y" %in% names(daii_scored)) {
  daii_scored <- daii_scored %>%
    mutate(strategic_profile = coalesce(strategic_profile.y, strategic_profile.x)) %>%
    select(-strategic_profile.x, -strategic_profile.y)
}

# Remove any other .x .y columns if they exist
daii_scored <- daii_scored %>%
  select(-ends_with(".x"), -ends_with(".y"), -any_of(c("strategic_profile.x", "strategic_profile.y")))
...
cat("   Creating top anomalies portfolio...\n")

# Check which columns actually exist
available_cols <- names(daii_scored)
cat("   Available columns:", paste(head(available_cols, 10), collapse=", "), "...\n")

# Build select list dynamically based on available columns
select_cols <- c("ticker", "ai_score", "innovation_score", "anomaly_score")
if("strategic_profile" %in% available_cols) select_cols <- c(select_cols, "strategic_profile")
if("rd_intensity" %in% available_cols) select_cols <- c(select_cols, "rd_intensity")
if("patent_activity" %in% available_cols) select_cols <- c(select_cols, "patent_activity")
if("total_fund_weight" %in% available_cols) select_cols <- c(select_cols, "total_fund_weight")
...
# Show what we got
cat("   Selected columns:", paste(names(top_anomalies_portfolio), collapse=", "), "\n")

# Top anomalies NOT in portfolio (Discovery opportunities)
cat("   Creating top anomalies discovery...\n")

# Build select list for discovery
discovery_cols <- c("ticker", "ai_score", "innovation_score", "anomaly_score")
if("strategic_profile" %in% available_cols) discovery_cols <- c(discovery_cols, "strategic_profile")
if("rd_intensity" %in% available_cols) discovery_cols <- c(discovery_cols, "rd_intensity")
if("patent_activity" %in% available_cols) discovery_cols <- c(discovery_cols, "patent_activity")
if("discovery_tier" %in% available_cols) discovery_cols <- c(discovery_cols, "discovery_tier")

top_anomalies_discovery <- daii_scored %>%
  filter(is_anomaly == TRUE & in_portfolio == FALSE) %>%
  arrange(desc(anomaly_score)) %>%
  select(all_of(discovery_cols))

cat("   Discovery columns:", paste(names(top_anomalies_discovery), collapse=", "), "\n")
...
if (exists("rf_model") && !is.null(rf_model)) {
  ...
  if(!"predicted_ai_leader" %in% names(daii_scored)) {
    ...
    if(na_count > 0) {
      cat("   ⚠️ Found", na_count, "NA values in features. Imputing with column means...\n")
      ...
      if(length(all_na_cols) > 0) {
        cat("   ⚠️ Columns still have NAs after imputation:", 
  ...
  if(length(target) != nrow(daii_scored)) {
    cat("   ⚠️ Target length mismatch, recreating target...\n")
    target <- factor(ifelse(daii_scored$ai_quartile == 4, "Leader", "Other"))
  }
  ...
  if(length(daii_scored$ticker) == length(target) && 
    ...
    cat("   ⚠️ Length mismatch after fixes. Creating simplified output.\n")
...
if("anomaly_score" %in% names(daii_scored)) {
  ...
  na_count <- sum(is.na(daii_scored$anomaly_score))
  if(na_count > 0) {
    cat("   ⚠️ Found", na_count, "NA values in anomaly_score. Removing for visualization.\n")
  ...
  if(length(anomaly_scores_clean) > 0) {
    ...
    cat("   ⚠️ No valid anomaly scores for histogram\n")
  }
  ...
  if(all(c("innovation_score", "ai_score", "is_anomaly", "in_portfolio") %in% names(daii_scored))) {
    ...
    if(nrow(plot_data) > 0) {
      ...
      cat("   ⚠️ No valid data for scatter plot\n")
    }
  } else {
    cat("   ⚠️ Missing required columns for scatter plot\n")
  }
  
} else {
  cat("   ⚠️ anomaly_score column not found. Skipping visualizations.\n")
}
...
# =============================================================================
# TEST NEW AZURE CONNECTIONS
# =============================================================================
if(interactive() && FALSE) {  # Set to TRUE to test
  cat("\n", paste(rep("=", 60), collapse = ""), "\n")
  cat("🔐 TESTING AZURE CONNECTIONS\n")
  cat(paste(rep("=", 60), collapse = ""), "\n")
  
  # Test holdings
  conn1 <- try(connect_holdings())
  if(!inherits(conn1, "try-error")) {
    cat("✅ Holdings connection successful\n")
    dbDisconnect(conn1)
  }
  
  # Test refinitiv
  conn2 <- try(connect_refinitiv())
  if(!inherits(conn2, "try-error")) {
    cat("✅ Refinitiv connection successful\n")
    dbDisconnect(conn2)
  }
}
